...
int main()
{
    ...
    participant_data_array = (participant_data *) malloc(MAXPART * sizeof(meeting_data));  // "participant data" is a struct
    odas_data_array = (odas_data *) malloc (NUM_CHANNELS * sizeof(odas_data));  // "odas data" is a struct
  ...
  while (1)
  {
    ...
    while (timestamp_last_target <= timestamp_last_category)
    {
      ...
      if (bytes_returned > 0)
      {
        timestamp_last_target = json_parse(target_buffer, odas_data_array, 0);
      }
    }
    ...
    while (timestamp_last_category < timestamp_last_target)
    {
      ...
      if (bytes_returned > 0)
      {
        timestamp_last_category = json_parse(category_buffer, odas_data_array, 0)
      }
    }

    output_buffer = process_sound_data(&meeting_data, participant_data_array, odas_data_array);
    bufferSize = strlen(output_buffer);
    // send out UDP message to app
    sendto(app_sockfd, output_buffer, bufferSize, 0, (struct sockaddr *) &app_addr, sizeof(struct sockaddr_in));
...
char * process_sound_data(meeting *  meeting_data, participant_data * participant_data_array, odas_data * odas_data_array,)
    ...
    for (iChannel = 0; iChannel < NUM_CHANNELS; iChannel++)
    {
        ...
        if (odas_data_array[iChannel].activity > MINENERGY)
        {
            ...
            if (meeting_data->angle_array[target_angle] == 0x00 && meeting_data->num_participants < (MAXPART - 1))
            {
                ...
                printf ("new person %d\n", num_participants);
                meeting_data->participant_angle[num_participants] = target_angle;
                
                // set intial frequency high
                participant_data_array[num_participants]->participant_frequency = 200.0;
                ...
                participant_data_array[num_participants].participant_is_talking = 10 * energy[iChannel];
            }
            else // its an existing talker we're hearing
            {
                // could put logic in here to count turns
                participant_data_array[meeting_data->angle_array[target_angle]].participant_is_talking = 10 * energy[iChannel];
...
void initialise_meeting_data(meeting * meeting_data, participant_data * participant_data_array, odas_data * odas_data_array) {
    ...
    for (i=0;i<MAXPART;i++) {
        meeting_data_array[i].participant_angle = 0;
        meeting_data_array[i].participant_is_talking= 0;
        meeting_data_array[i].participant_silent_time= 0;
        meeting_data_array[i].participant_total_talk_time= 0;
        meeting_data_array[i].participant_num_turns= 0;
        meeting_data_array[i].participant_frequency= 150.0;        
    }
